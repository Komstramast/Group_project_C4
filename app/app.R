...
server <- function(input, output) {
  ...
  # === Главная: гистограмма LoC ===
  output$locHistogram <- renderPlot({
    ggplot(commits, aes(x = loc_change)) +
      geom_histogram(bins = 50, fill = "steelblue", color = "white") +
      labs(
        title = "Распределение изменений строк кода (LoC) по коммитам",
        x = "LoC (измененные строки)",
  ...
  output$anomalyHoursPlot <- renderPlot({
    anomalies %>%
      ...
      labs(
        title = "Часы активности аномальных коммитов",
        x = "Час",
        y = "Кол-во аномалий"
      ) +
      theme_minimal()
